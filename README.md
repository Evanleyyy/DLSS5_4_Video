...
当前代码版本 **0.4.1**：已把 30 系分支的兼容运行库选择整合回原项目。30 系优先自动选择 SF-v2，40 系保留原版运行库；两层可分别选择并保存到参数预设。原项目名称、安装 AppId、默认目录、用户设置和其他超分功能继续沿用。详见 [0.4.1 更新说明](docs/0.4.1更新说明.md)。
...
下列链接是已发布的 **0.3.7** 安装包，不包含 0.4.1 新增的 30 系能力。0.4.1 当前可从本分支源码启动或按 [打包说明](packaging/打包说明.md)构建，本次合并不替换已发布附件。
